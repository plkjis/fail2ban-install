...
# [IMPROVEMENT] Use cat with a heredoc for cleaner syntax.
cat <<EOF > "${CONFIG_FILE}"
...
echo -e "${C_BLUE}>>> 正在测试 Fail2ban 配置...${C_RESET}"
# [IMPROVEMENT] Test configuration before restarting.
if ! fail2ban-client -d | grep -q "sshd"; then
    echo -e "${C_RED}✖ 错误: Fail2ban 配置测试失败。${C_RESET}"
    echo -e "${C_YELLOW}这通常意味着存在一个损坏的配置文件 (例如，在 /etc/fail2ban/filter.d/ 中缺少 [Definition])。${C_RESET}"
    echo -e "${C_YELLOW}请运行 'fail2ban-server -f' 来查找具体错误。${C_RESET}"
    exit 1
fi
echo -e "${C_GREEN}✔ 配置测试通过。${C_RESET}"
